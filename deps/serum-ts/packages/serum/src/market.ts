...
export class Market {
  ...
  */
  ...
  async makePlaceOrderTransaction<T extends PublicKey | Account>(
    ...
    await this.findOpenOrdersAccountsForOwner(
      connection,
      ownerAddress,
      cacheDurationMs,
    );
  ...
  makeNewOrderV3Instruction_accounting4fees<T extends PublicKey | Account>(
    params: OrderParams<T>,
  ): TransactionInstruction {
    const {
      owner,
    ...
    return DexInstructions.newOrderV3({
      ...
      // on buy, add fees of 0.0004
      maxQuoteQuantity:
        side == 'buy'
          ? new BN(this._decoded.quoteLotSize.toNumber())
              .mul(
  ...
  makeNewOrderV3Instruction__market_hack<T extends PublicKey | Account>(
    params: OrderParams<T>,
  ): TransactionInstruction {
    const {
      owner,
    ...
    // @ts-ignore
    const ownerAddress: PublicKey = owner.publicKey ?? owner;
    const awayFromMarketMultiplier = 1.2;
    return DexInstructions.newOrderV3({
      market: this.address,
      bids: this._decoded.bids,
      asks: this._decoded.asks,
      ...
      limitPrice:
        side == `sell`
          ? new BN(1)
          : this.priceNumberToLots(price * awayFromMarketMultiplier),
      maxBaseQuantity: this.baseSizeNumberToLots(size),
      maxQuoteQuantity:
        side == `sell`
          ? new BN(1)
